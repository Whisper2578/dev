...
def scrabble(word):
    """ Функция-помошник для игры "Эрудит".

    :param word: Строка (слово)
    :return: Количество очков из расчёта:
    (а, в, е=ё, и, н, о, р, с, т ) - 1 очко, (д, к, л, м, п, у) - 2 очка, (б, г, ь, я) - 3 очка, (й, ы) - 4 очка,
    (ж, з, х, ц, ч) - 5 очков, 6 и 7 очков не предусмотрено, (ф, ш, э, ю) - 8 очков, (щ) - 10 очков, (ъ) - 15 очков.
    """

    dict_points = {1:'авеёинорст', 2:'дклмпу', 3:'бгья', 4:'йы', 5:'жзхцч', 8:'фшэю', 10:'щ', 15:'ъ'}
    # points = sum([k for i in word for k, v in dict_points.items() if i in v])
    result = 0
    for letter in word:
        for point, letters in dict_points.items():
