...
def everything_for_your_cat(cats_data):
    """ Функция оптимизаиии хранения данных питомцев и их владельцев. На вход подаётся список картежей, где каждый
        кортеж состоит из: Кличка котика, Возраст котика, Имя покупателя, Фамилия покупателя. Имена некоторых владельцев
        повторяются, потому что у них несколько кошек. На выходе получаем данные по каждому владельцу отдельной строкой
        формата "Имя_покупателя Фамилия_покупателя: Кличка_котика1, Возраст; ...; Кличка_котикаN, Возраст\n" """
